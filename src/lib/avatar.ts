...
async function fetchWearable(urn: string, env: Env) {
  return nftApi.fetchWearable(urn, env).catch((error: Error) => console.log(`Failed to load wearable="${urn}"`, error))
}
...
async function fetchProfile(profile: string, env: Env) {
  ...
  return nftApi
    ...
    .then((profile: { avatars: string | any[] }) => (profile && profile.avatars.length > 0 ? profile.avatars[0] : null))
...
async function fetchWearableFromContract(options: { contractAddress: string; itemId?: string | null; tokenId?: string | null; env: Env }) {
  ...
  let cItemId = `${contractAddress}-${itemId}`
  if (tokenId && !itemId) {
    const nft = await nftApi.fetchNFT(contractAddress, tokenId, env)
    cItemId = `${contractAddress}-${nft.collectionItemId}`
  }
  return fetchWearable(cItemId, env)
}
